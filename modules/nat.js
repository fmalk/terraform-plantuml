export function loadNAT(state, stack, subnet_id) {
  const records = state.resources.filter((r) => r.type === 'aws_nat_gateway' && r.instances[0].attributes.subnet_id === subnet_id);
  records.forEach((nat, idx) => {
    const atts = nat.instances[0].attributes;
    stack.push({
      isGroup: false,
      title: `${atts.tags.Name || atts.id}\\n${atts.private_ip}\\n${atts.public_ip}`,
    ...
    const route_tables_ids = state.resources
      .filter((r) => r.type === 'aws_route_table' && r.instances[0].attributes.route.some((t) => t.nat_gateway_id === atts.id))
